...
----
. By default Spring Boot will only exposes `health` and `info` endpoint over http. We will expose rest of the endpoints for this lab. **But** in production you would not expose all these endpoints and additionally secure them.  You can read more about that in the https://docs.spring.io/spring-boot/docs/current/reference/html/production-ready-endpoints.html[documentation]. In this lab, for simplicity, we will expose them all without securing. 
+
...
. Browse to the http://localhost:8080/actuator/info[/actuator/info] endpoint. Git commit information is now included:
+
[source,json]
----
{
...
*What Just Happened?*
+
By including the `git-commit-id-plugin`, details about git commit information will be included in the http://localhost:8080/actuator/info[/actuator/info] endpoint. Git information is captured in a `git.properties` file that is generated with the build.
+
...
----
+
These will add the project’s Maven coordinates to the http://localhost:8080/actuator/info[/actuator/info] endpoint. The Spring Boot Maven plugin will cause them to automatically be replaced in the assembled JAR.
+
...
. Browse to the http://localhost:8080/actuator/info[/actuator/info] endpoint. Build information is now included:
+
[source,json]
----
{
...
*What Just Happened?*
+
We have mapped Maven properties from the `pom.xml` into the http://localhost:8080/actuator/info[/actuator/info] endpoint.
+
Read more about exposing data in the http://localhost:8080/actuator/info[/actuator/info] endpoint http://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#production-ready[here]

== Health Indicators

Spring Boot provides a http://localhost:8080/actuator/health[/actuator/health] endpoint that exposes various health indicators that describe the health of the given application.

Normally, when Spring Security is not enabled, the http://localhost:8080/actuator/health[/actuator/health] endpoint will only expose an UP or DOWN value.
...
. Browse to the http://localhost:8080/actuator/health[/actuator/health] endpoint. Out of the box is a `DiskSpaceHealthIndicator` that monitors health in terms of available disk space. Would your Ops team like to know if the app is close to running out of disk space? `DiskSpaceHealthIndicator` can be customized via `DiskSpaceHealthIndicatorProperties`. For instance, setting a different threshold for when to report the status as DOWN.
+
[source,json]
----
{
...
. Browse to the http://localhost:8080/actuator/health[/actuator/health] endpoint and verify that the output is similar to the following (and changes randomly!).
+
[source,json]
----
{
...
Spring Boot provides a http://localhost:8080/actuator/metrics[/actuator/metrics] endpoint that exposes several automatically collected metrics for your application. It also allows for the creation of custom metrics.

. Browse to the http://localhost:8080/actuator/metrics[/actuator/metrics] endpoint. Review the metrics exposed:
+
[source,json]
----
{
...
----
+
This list all the metrics available via this endpoint. For example, to see the `jvm.memory.used` metric, navigate to http://localhost:8080/actuator/metrics/jvm.memory.used[/actuator/metrics/jvm.memory.used] url. You will see an output like
+
[source,json]
----
{
...
[NOTE]
====
Application health will flip between UP and DOWN frequently. This is due to the `FlappingHealthIndicator` that we created. This is an expected behaviour.
====
...
Lets proceed to link:../lab04/lab04.adoc[Lab 4] or back to link:../../README.adoc[Home]
