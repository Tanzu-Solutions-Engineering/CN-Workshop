...
. If you prefer the CLI you can create the service instance as follows.
+
[source.bash]
----
cf create-service p.config-server standard config-serve
----
+
...
[source,bash]
----
cf update-service config-server -c '{"git": { "uri": "https://github.com/Pivotal-Field-Engineering/CN-Workshop", "label": "config" } }'
----
